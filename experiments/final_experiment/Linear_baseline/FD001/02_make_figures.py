"""
02_make_figures.py — Linear regression baseline, FD001: fleet-level
predicted-vs-true RUL scatter.

============================================================================
WHY ONE FLEET SCATTER, NOT 100 PER-ENGINE FIGURES
============================================================================
Unlike PF_Fixed_Parameters/PF_PMMH/library_method (one figure per test
engine, showing a predicted RUL *trajectory* across every observed
cycle), LinearRULBaseline produces exactly ONE point prediction per
engine, from its last observed cycle only -- there is no per-cycle
trajectory to plot, and no credible interval to shade. A per-engine
panel would therefore be a single dot, which carries no more information
than its row in tables/rul_per_engine.csv. The fleet-level scatter below
(one dot per engine, predicted vs. true RUL, with the y=x reference line)
is the figure that actually shows something: how far off the pooled OLS
fit is, and whether the error is systematic (fleet above/below y=x) or
scattered.

Refits LinearRULBaseline here rather than reading predictions back from
results.db -- OLS on fixed data is deterministic and near-instant (see
01_run_experiment.py's fit_time print), so recomputing it is the same
"cheap, deterministic step -> just recompute" convention every sibling
script uses for its own pooled model (e.g.
PF_Fixed_Parameters/FD001/02_make_figures.py refits pooled_model to
recompute observed_hi rather than persisting it).

Run from the repo root:
    python experiments/final_experiment/Linear_baseline/FD001/02_make_figures.py
"""
...
FIG_DIR = "experiments/final_experiment/Linear_baseline/FD001/figures"
