"""
03_make_tables.py — PF Fixed-Parameters experiment, FD001: fleet-level
summary tables for Health Index and RUL, read back from results.db.

Same table schema, columns, and metric definitions as
library_method/FD001/04_make_tables.py's TABLE 1/TABLE 2 (see that
script's module docstring for the trajectory-level-vs-final-cycle
granularity rationale) -- a "strategy" column is kept, holding the single
value "fixed_parameters" rather than one_shot/periodic, so this table can
be concatenated directly with the library method's for a side-by-side
comparison.

OUTPUT
============================================================================
  - tables/health_index_summary.csv     (1 row: fixed_parameters)
  - tables/rul_summary.csv               (1 row: fixed_parameters)
...
Run from the repo root:
    python experiments/final_experiment/PF_Fixed_Parameters/FD001/03_make_tables.py
"""
...
SIGMA_0 = 0.05
DB_PATH = "results.db"
CONFIG_JSON = "experiments/final_experiment/PF_Fixed_Parameters/FD001/config.json"
TABLE_DIR = "experiments/final_experiment/PF_Fixed_Parameters/FD001/tables"

STRATEGY = "fixed_parameters"
...
def main():
    ...
    print()
    print("=" * 78)
    print("TABLE 2 — RUL (final-cycle, fixed-parameters pooled PF)")
    print("=" * 78)
