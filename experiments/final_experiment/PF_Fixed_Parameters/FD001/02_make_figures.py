"""
02_make_figures.py — PF Fixed-Parameters experiment, FD001: per-engine
health index + RUL figure, read straight back out of results.db.
...
Same two building blocks as library_method/FD001/03_make_figures.py
(particle_twin.visualization.plots.plot_health_tracking /
plot_rul_distribution), via plots.engine_summary_figure() -- ONE column
rather than a one-shot/periodic 2x2 grid, because "fixed parameters" is a
single pooled-model run per engine; there is no second strategy to put in
a second column.
...
same convention as library_method's figure step.

Run from the repo root:
    python experiments/final_experiment/PF_Fixed_Parameters/FD001/02_make_figures.py
"""
...
FIG_DIR = "experiments/final_experiment/PF_Fixed_Parameters/FD001/figures"
...
def main():
    ...
    for engine_id in all_engines:
        ...
        fig = engine_summary_figure(
            ...
            extra_title=f"Fixed parameters (pooled model) — n_cycles={n_cycles}, "
