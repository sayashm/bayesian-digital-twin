"""
01_run_experiment.py — PF Fixed-Parameters experiment, FD001: run the
bootstrap particle filter with ONE pooled ("fixed") set of degradation
parameters over every FD001 test engine, using the v2 RUL extraction
...
calibration settings) as library_method/FD001/02_run_experiment.py, so the
...
============================================================================
  ...
  - experiments/final_experiment/PF_Fixed_Parameters/FD001/config.json:
...
Run from the repo root (~2-3 minutes for all 100 engines, n_particles=500):
    python experiments/final_experiment/PF_Fixed_Parameters/FD001/01_run_experiment.py
"""
...
# library_method/FD001/02_run_experiment.py so the two experiments are
# directly comparable.
# ---------------------------------------------------------------------------
...
def main():
    ...
    if "exp_id" in config:
        ...
        exp_id = db.create_experiment(
            ...
            extra_params={
                "step": "final_experiment/PF_Fixed_Parameters/FD001/01_run_experiment",
                "max_horizon": MAX_HORIZON, "rul_cap": RUL_CAP,
                "mu_fail": mu_fail, "sigma_fail": sigma_fail, "threshold_mode": "posterior",
            },
            description="Final experiment (PF fixed parameters, FD001) — one pooled "
