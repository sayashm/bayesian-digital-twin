"""
04_health_vs_rul.py — quick diagnostic: how does Health Index relate to
RUL, for TRAIN vs. TEST (both ground truth -- no model predictions read
back from results.db here)?

Identical in content to PF_Fixed_Parameters/FD003/04_health_vs_rul.py /
library_method/FD003/05_health_vs_rul.py (same pooled-model config, same
10 representative engines) -- this diagnostic is about the deterministic
observed Health Index vs. ground-truth RUL, so it does not depend on which
downstream method (library matching, fixed parameters, or PMMH-calibrated
pooled parameters) will later be used to predict RUL from it. Kept as its
own copy here (rather than a shared import) so PF_PMMH/FD003/ is
self-contained and its own figures/ directory is the one actually written
to.

10 engines: unit_id 1, 10, 20, ..., 90.
  - Train engines run to failure, so the RUL at any cycle is just
    (max_cycle - cycle) -- no model needed.
  - Test engines are truncated before failure. RUL_FD003.txt gives the
    ...
    particle_twin.analysis.metrics / 03_make_tables.py).
...
Run from the repo root:
    python experiments/final_experiment/PF_PMMH/FD003/04_health_vs_rul.py
"""
...
TRAIN_OUT_PATH = "experiments/final_experiment/PF_PMMH/FD003/figures/health_vs_rul_train.png"
TEST_OUT_PATH = "experiments/final_experiment/PF_PMMH/FD003/figures/health_vs_rul_test.png"
...
fig_train.suptitle("Health Index vs. RUL — train (actual)", fontsize=14)
plt.tight_layout()
...
for ax, engine_id in zip(axes_test.flat, ENGINES):
    ...
    # actual RUL, reconstructed from RUL_FD003.txt's truncation-point value.
...
fig_test.suptitle("Health Index vs. RUL — test (actual)", fontsize=14)
plt.tight_layout()
