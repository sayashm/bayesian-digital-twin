"""
01_run_experiment.py — PF PMMH experiment, FD003: run the bootstrap
...
============================================================================
PF_Fixed_Parameters/FD003/01_run_experiment.py fits ONE DegradationModel on
...
library_method/FD003/02_run_experiment.py goes to the opposite extreme:
...
computed and persisted by library_method/FD003/01_build_library.py
...
are identical to PF_Fixed_Parameters/FD003/01_run_experiment.py, so the
...
============================================================================
  ...
  - experiments/final_experiment/PF_PMMH/FD003/config.json:
...
excludes the PMMH library build, which is already done):
    python experiments/final_experiment/PF_PMMH/FD003/01_run_experiment.py
"""
...
# run in this project; RUL settings match PF_Fixed_Parameters/FD003's
...
LIBRARY_CONFIG_JSON = "experiments/final_experiment/library_method/FD003/config.json"
...
def _pmmh_pooled_params(db: ResultsDB) -> tuple[float, float, int, dict]:
    """
    Fleet-averaged (growth_rate, sigma_v) from library_method/FD003's
    already-built PMMH-overlaid library (results.db's engine_parameters
    table, exp_id read from library_method/FD003/config.json). Averaging
    ...
    if not records:
        raise RuntimeError(f"No engine_parameters rows for library_exp_id={library_exp_id} -- "
                            f"run library_method/FD003/01_build_library.py first.")
...
def main():
    ...
    if "exp_id" in config:
        ...
        exp_id = db.create_experiment(
            ...
            extra_params={
                "step": "final_experiment/PF_PMMH/FD003/01_run_experiment",
            ...
            description="Final experiment (PF PMMH, FD003) — one pooled DegradationModel "
