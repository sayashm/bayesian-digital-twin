"""
02_run_experiment.py — FD003 final experiment, step 2: run BOTH similarity
strategies (one-shot and periodic) over every FD003 TEST engine, and
store the full per-cycle health-index AND RUL predictions in results.db.

============================================================================
WHAT THIS SCRIPT DOES
============================================================================
Loads the FD003 training library built by 01_build_library.py
(config.json's "library_exp_id"), then for EVERY one of the 100 FD003 test
engines, runs particle_twin.library.SimilarityStreamingFilter TWICE:
...
n_particles=500 -- requires FD003/01_build_library.py to have completed
first, since FD003's PMMH calibration has no pre-existing exp_id to reuse
and takes ~3 hours on its own):
    python experiments/final_experiment/library_method/FD003/02_run_experiment.py
"""
...
# docstring (4) / DEFAULT_RUL_CAP=125), but is set well above the
# literature-recommended 125 here -- a deliberate experiment-specific
# choice to see the effect of a much looser cap, NOT the thesis's
# recommended default. Value set from the FD003 IDA (IDA_FD003.ipynb,
# cell 16 / cycle_length_stats.csv): the longest-lived FD003 TRAINING
# engine runs 525 cycles, i.e. a max training RUL of 524 (RUL at cycle 1
# = max_cycle - 1) -- 525 keeps both constants safely >= that value, up
# from FD001's 350 (FD001's own longest training engine runs only 362
# cycles). Since RUL_CAP>=MAX_HORIZON, the cap never actually binds here
...
RUL_CAP = 525
MAX_HORIZON = 525        # whichever of RUL_CAP/MAX_HORIZON is smaller binds first
SEED = 42
...
def main():
    ...
    print(f"[OK] Calibrated failure threshold (posterior space): "
          ...
          f"(FD003 has no equivalent pre-calibrated module constant -- this IS the calibration)")
    ...
    for strategy in STRATEGIES:
        ...
        exp_id = db.create_experiment(
            ...
            extra_params={
                "step": "final_experiment/FD003/02_run_experiment", "strategy": strategy,
            ...
            description=f"Final experiment (FD003) — {strategy} similarity matching "
